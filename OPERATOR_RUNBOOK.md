...
3. **NORMALIZE** — 出典URL、取得日時、ライセンス、利用条件、依存、費用、更新状況を記録する。
4. **FILTER** — `No License`、商用利用不可、費用不明、規約上危険、個人情報必須、維持負担過大を除外する。
5. **COMPOSE** — 実際に残った資産を2〜4個組み合わせ、顧客・問題・出力物・差別化を定義する。
6. **VALIDATE** — 支払い・売上、契約・発注、求人、有料レビュー、困りごとの証拠をWebで確認する。
7. **SCORE** — 指定の100点配点で計算する。競合存在だけでは需要点を加算しない。
8. **FALSIFY** — TOP5すべてについて、無料代替、大手参入、規約、法律、著作権、維持負担、価格競争を確認する。
9. **BUILD GATE** — 証拠・ライセンス・0円条件・反証を通過した1位だけを採用する。条件不足なら`NO BUILD`。
10. **BUILDER / QA** — 最初の1円を検証できる最小MVPを実装し、実際にテストする。
11. **MONETIZE / HANDOFF** — 価格、販売チャネル、納品方法、Kill Criteria、次の操作1つを提示する。
